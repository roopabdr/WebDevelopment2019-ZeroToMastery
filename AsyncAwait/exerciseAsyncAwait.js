...
const getData = async function() {
  const [ users, posts, albums ] = urls;
  const userResp = await fetch(users);
  const postsResp = await fetch(posts);
  const albumsResp = await fetch(albums);
  ...
  console.log('posta', postsData);
  console.log('albums', albumsData);
}

/*better version below by Andrei!*/
const getData = async function() {
